    ...
    def _clicked(self, event=None):
        if self._state != tkinter.DISABLED:
            ...
            if self._command is not None:
                if self._cmdargs not in (None,False) and self._cmdkwargs not in (None,False): self._command(*self._cmdargs,**self._cmdkwargs)
                elif self._cmdargs not in (None,False) and self._cmdkwargs in (None,False): self._command(*self._cmdargs)
                elif self._cmdargs in (None,False) and self._cmdkwargs not in (None,False): self._command(*self._cmdkwargs)
                else: self._command()
