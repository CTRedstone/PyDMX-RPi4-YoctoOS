...
def genLogLine(prc:str,tpe:str,msg:str,debug=True,errhandling="yellow"):
	tpe = tpe.lower()
	color = FE["cyan"] if tpe in ("inf","info") else FE["blue"] if tpe == "debug" and debug == True else FE["green"] if tpe in ("ok","okay","done") else FE["yellow"] if tpe in ("warn","warning") or (errhandling == "yellow" and tpe in ("err","error")) else FE["red"] if tpe == "fatal" or (errhandling == "red" and tpe in ("err","error")) else FE["purple"] if tpe in ("tip","tipp") else ""
