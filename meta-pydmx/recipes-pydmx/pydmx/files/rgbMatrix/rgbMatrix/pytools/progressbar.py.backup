from string import adjustStrLen
class progressbar:
	...
	def draw(self):
		styles = ["==","=>",">>","--","->","||","|>"]
		print(f"{adjustStrLen(self.text)} [{styles[self.style][0]*self.var+styles[self.style][1]}{' '*(self.steps-self.var)}]",end="\r")
	def update(self,**args):
		"""
		You can update: text, style, change (-> add to bar)
		"""
		try: self.text = args["test"]
