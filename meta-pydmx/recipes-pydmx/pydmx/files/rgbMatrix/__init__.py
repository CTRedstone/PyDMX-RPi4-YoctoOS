...
class matrixDisplay:
	...
	def __getattr__(self,name):
		...
		if name in self.originAttribs and name in dir(self): pass
		if name not in self.originAttribs or (self.quitted == True and name in self.originAttribs): raise cheatingException(f"The function '{name}' wasn't defined when matrixDisplay got initialized, but now it exists. What do you think: Is cheating ok?")
		if name in self.originAttribs: raise AttributeError(f"The function '{name}' you're trying to execute got deleted, probably by the matrixDisplay.quit() function. To use matrixDisplay again, you'll have to create a new matrixDisplay()")
		else: raise AttributeError(f"matrixDisplay has no attribute called '{name}'")

	def log(self,mth:str,tpe:str,msg:str,errhandling="red",eol="\n"):
		if self.debug or tpe.lower() != "debug": print(gLL(f"{self.masterMod+'/' if self.masterMod != '' else ''}rgbMatrix/matrixDisplay{'/'+mth if mth != '' else ''}",tpe,msg,debug=self.debug,errhandling=errhandling),end=eol)
	...
	def __init__(self,rows=32,columns=64,chain_length=1,parallel=1,hardware_mapping="regular",disable_hardware_pulsing=True,brightness=50,masterMod="",debug=False,replaceIfUnavail=True,forceReplace=False):
		...
		#If the display is available (Imports are ok and forceReplace is False) It will use the normal Display function, otherwise tkinter will be initialized
		if svars.available:
	...
	def quit(self): #Clears the display, but if the tkinter window is used, it gets removed
		if svars.available:
			self.clearDisp()
		elif self.replace:
			...
			try: self.dispRepWin.update()
			except: pass

		class errors():
			class invailidTypeError(Exception): ...
			class unallowedFunctionException(Exception): ...

		self.quitted = True
		rfc = self.__class__
		cnt = 0
		if not isinstance(rfc,type):
			self.log(cFN(),"fatal",f"Deleting functions of display class failed because the class reference of matrixDisplay has to be a class, not {type(rfc)}")
			raise invailidTypeError(f"Could not quit matrixDisplay: class reference has to be a class, not {type(rfc)}")
		"""elif hasattr(rfc,"__dir__"):
			self.log(cFN(),"fatal",f"Deleting functions of display class failed because the class reference has an __dir__ attribute which will cause issues in the code")
			raise unallowedFunctionException(f"Could not quit matrixDisplay: matrixDisplay contains a __dir__ method")"""

		for elm in dir(rfc):
			self.log(cFN(),"debug",f"Processing elm {elm} for remove...                             ",eol="\r")
			if (elm[0] != "_" or elm == "__init__" or elm == "quit") and elm != "log" and hasattr(rfc,elm) and callable(getattr(rfc,elm)):
				delattr(rfc,elm)
				cnt += 1
		if cnt < 5:
			self.log(cFN(),"warn",f"Deleting functions of display class did not remove the minimum of functions - {cnt} were removed")
		else:
			self.log(cFN(),"ok",f"Sucessfully quit matrixDisplay class - removed {cnt} functions")

	def renderDisp(self,no_log=False): #Renders the display
		if svars.available:
	...
	def setPixel(self,pos=(0,0),color=(255,255,255),directDraw=False,no_log=False): #Changes the color of a specific pixel
		...
		if svars.available and type(color) == tuple:
			...
			x,y = pos
			r,g,b = color
			try: self.cnv.SetPixel(x,y,r,b,g)
	...
	def generateImgFromJson(self,filepath="",unlistedPixelColor=(0,0,0),startAt=(0,0),directDraw=False,ignoreUnlistedPixel=True,ignoreBlackPixel=False): #Displays an image from a simple .json file (see resources/RedstoneDust32x32.jsonimg)
	...
	def generateSquare(self,pos1=(0,0),pos2=(0,0),color=(255,255,255),directDraw=False,directDrawLvl=2): #Generates a square on the screen
		"""
	...
	def generateImage(self,filepath="",size=(64,32),startAt=(0,0),directDraw=False): #Displays an image from a normal image file
		if svars.available or self.replace:
			if filepath == "":
	...
	def generateImageFromPIL(self,image,startAt=(0,0),directDraw=False,no_log=False): #Displays an image from an already existing PIL object
		if svars.available or self.replace:
	...
	def generateText(self,text="",font="/usr/share/fonts/truetype/freefont/FreeMono.ttf",font_size=24,text_color=(255,255,255),bg_color=(0,0,0),directDraw=False,no_log=False):
		if svars.available or self.replace:
			...
			self.log(cFN(),"debug","Generating text")
			try:
				...
				return
			self.log(cFN(),"ok","Generated text")
			self.generateImageFromPIL(img,directDraw=directDraw)
		else: self.notAvail(cFN())

	#Generates a text which can scroll on the screen
	...
	#Generates a multiline text on the display
	def generateMultiLineText(self,textpos=(0,0),text="",text_color=(255,255,255),bg_color=(0,0,0),font_size=24,font="/usr/share/fonts/truetype/freefont/FreeMono.ttf",align="center",line_spacing=4,no_log=False,directDraw=True):
		if svars.available or self.replace:
			if not no_log: self.log(cFN(),"info",f"Generating text '{text}' with font '{font}' in font size {font_size} in color {self.tools.tplClr(text_color)} with background color {self.tools.tplClr(text_color)} starting at {self.tools.tplPos(textpos)}")
			...
			self.log(cFN(),"debug","Generating text background")
			try:
				_draw = ImageDraw.Draw(PIL.Image.new("RGB",(self.columns-textpos[0],self.rows-textpos[1]),bg_color))
				text_width, text_height = _draw.multiline_textbbox(textpos,text,font=_font,spacing=line_spacing)[2:4]
				img = PIL.Image.new("RGB",(self.columns-textpos[0],self.rows-textpos[1]),bg_color)
				_draw = ImageDraw.Draw(img)
			except Exception as exc:
				self.log(cFN(),"err",f"Failed to generate background of text: {exc}")
				return
			...
			self.log(cFN(),"debug","Generating text")
			try:
				_draw.text(((64-textpos[0]-text_width) // 2, (32-textpos[1]-text_height) // 2), text, font=_font, fill=text_color)
			except Exception as exc:
				self.log(cFN(),"err",f"Failed to generate text onto background: {exc}")
				return

			if not no_log: self.log(cFN(),"ok","Generated text")
			self.generateImageFromPIL(img,directDraw=directDraw,no_log=no_log)
		else: self.notAvail(cFN())

	#clears the display
	def clearDisp(self,no_log=False,directDraw=True):
		if svars.available:
			if not no_log: self.log(cFN(),"warn","Clearing display")
			for i in range(5):
				self.cnv.Clear()
				if directDraw: self.renderDisp(no_log=no_log)
			if not no_log: self.log(cFN(),"ok","Cleared display")
		elif self.replace:
			if not no_log: self.log(cFN(),"warn","Clearing display")
	...
	def clear(self,no_log=False,directDraw=True):
		self.clearDisp(no_log=no_log,directDraw=directDraw)
